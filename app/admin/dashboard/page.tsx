...
export default function DashboardPage() {
  ...
  const [prodPeriod, setProdPeriod] = useState('7d');
  const [salesPeriod, setSalesPeriod] = useState('7d');
  const [attPeriod, setAttPeriod] = useState('7d');
  ...
  const filterData = (dataArray: any[], period: string) => {
    if (!dataArray || !Array.isArray(dataArray)) return [];
    let daysToInclude = 3650;
    ...
    const cutoffTime = new Date().getTime() - daysToInclude * 24 * 60 * 60 * 1000;
    return dataArray.filter((d: any) => new Date(d.date).getTime() >= cutoffTime);
  };
  ...
  let { metrics, chartData, recentProduction, recentSales, todayAbsentees } = data;
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '36px' }}>
        <h3 style={{ fontSize: '18px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '16px' }} className="animate-fadeup">
          Egg Godown Analytics
        </h3>
        ...
        <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', padding: '24px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            <CustomSelect
              value={prodPeriod}
              onChange={(val) => setProdPeriod(val)}
              options={[
        ...
        <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', padding: '24px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            ...
            <CustomSelect
              value={salesPeriod}
              onChange={(val) => setSalesPeriod(val)}
              options={[
                { value: '7d', label: 'Last 7 Days' },
                { value: '1m', label: '1 Month' },
                { value: '3m', label: '3 Months' },
      ...
      <div>
        ...
        <div style={{ background: 'var(--white)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', padding: '24px', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>
            <div style={{ background: 'var(--green-light)', borderRadius: 'var(--radius-md)', padding: '16px' }}>
              <p style={{ fontSize: '12px', fontWeight: 600, color: '#3E6B22', marginBottom: '4px' }}>Active Workers</p>
              <p style={{ fontSize: '28px', fontWeight: 800, color: 'var(--primary)' }}>{metrics.labour?.active || 0}</p>
            </div>
            <div style={{ background: 'var(--champagne)', borderRadius: 'var(--radius-md)', padding: '16px' }}>
              <p style={{ fontSize: '12px', fontWeight: 600, color: '#A05A2C', marginBottom: '4px' }}>Present Today</p>
              <p style={{ fontSize: '28px', fontWeight: 800, color: 'var(--primary)' }}>{metrics.labour?.presentToday || 0}</p>
            </div>
            <div style={{ background: 'var(--pink)', borderRadius: 'var(--radius-md)', padding: '16px' }}>
              <p style={{ fontSize: '12px', fontWeight: 600, color: '#8B2E2E', marginBottom: '4px' }}>Absent Today</p>
              <p style={{ fontSize: '28px', fontWeight: 800, color: 'var(--primary)' }}>{metrics.labour?.absentToday || 0}</p>
            </div>
            <div style={{ background: 'var(--sky)', borderRadius: 'var(--radius-md)', padding: '16px' }}>
              <p style={{ fontSize: '12px', fontWeight: 600, color: '#1A6E8E', marginBottom: '4px' }}>Half Day Today</p>
              <p style={{ fontSize: '28px', fontWeight: 800, color: 'var(--primary)' }}>{metrics.labour?.halfDayToday || 0}</p>
            </div>
          </div>
        </div>
            ...
            {filterData(chartData, attPeriod).length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={filterData(chartData, attPeriod)} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="date" axisLine={false} tickLine={false} tick={false} dy={8} />
                  <YAxis axisLine={false} tickLine={false} width={45} tickFormatter={formatCompactNumber} tick={{ fontSize: 11, fill: 'var(--text-muted)', fontFamily: 'DM Sans, sans-serif' }} />
                  <Tooltip itemSorter={(item) => { const order: any = { present: 1, absent: 2, half: 3 }; return order[item.dataKey as string] || 0; }} labelFormatter={(label) => { if (typeof label === 'string' && label.split('-').length === 3) { const parts = label.split('-'); return `${parts[2]}-${parts[1]}-${parts[0]}`; } return label; }} cursor={{ fill: 'var(--grey-bg)', rx: 6 }} contentStyle={{ borderRadius: 10, border: 'none', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', fontFamily: 'DM Sans, sans-serif', fontSize: 12 }} />
                  <Area type="linear" dataKey="present" name="Present" stroke="#3E6B22" strokeWidth={3} fill="url(#colorPresent)" />
                  <Area type="linear" dataKey="absent" name="Absent" stroke="#8B2E2E" strokeWidth={3} fill="url(#colorAbsent)" />
                  <Area type="linear" dataKey="half" name="Half Day" stroke="#B45309" strokeWidth={3} fill="url(#colorHalf)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontSize: 13 }}>
